...
export default function Button({
  ...
  const classString: string = disabled
    ? `inline-block px-12 py-6 ${color} text-white font-medium text-xl leading-tight uppercase rounded-full shadow-md focus:outline-none focus:ring-0 transition duration-150 ease-in-out pointer-events-none opacity-60`
  ...
  return (
    <ConditionalWrapper
      ...
      wrapper={(children: ReactChild | ReactFragment) => (
        <Link to={to + `?locale=${locale}`}>{children}</Link>
      )}
    >
      <button {...rest} className={classString} disabled={disabled}>
        {text}
      </button>
    </ConditionalWrapper>
  );
